...
function extractContentIdentity(title: string, creator: string): string {
  const lower = title.toLowerCase()
  
  // For music: use artist name from creator
  // For series: try to extract show name (usually before | or - or Episode)
  const separators = [' | ', ' - ', ': ', ' — ']
  for (const sep of separators) {
    if (title.includes(sep)) {
      return title.split(sep)[0].trim().toLowerCase()
    }
  }
  
  // Fall back to creator name
...
export function selectRepresentativeVideos(items: ActivityItem[]): RepresentativeVideo[] {
  ...
  const scored: ScoredItem[] = unique.map(item => {
    ...
    const contentIdentity = extractContentIdentity(item.title, item.creator_name)
